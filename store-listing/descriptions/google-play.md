...
## Short description (max 80 characters — this one is exactly 80)

```
Self-hosted scheduling for Jehovah's Witnesses congregations. Your own Firebase.
```

## Full description (max 4000 characters — this one is ~2,320)
...
- Life and Ministry Meeting schedule, with direct .epub workbook import and a qualification-aware assignment picker that favors whoever was assigned longest ago
...
| Tags | scheduling, church, volunteer, community, calendar |
| Contact e-mail | see `../app-metadata.md` |
| Privacy policy URL | see `../app-metadata.md` |
| Ads | No ads |
| In-app products | None |

Re-check the character counts by pasting into Play Console's own counter
before publishing — copy only the text inside the code fences above (no
