...
class PwRecurringScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(l10n.pwRecurringRules),
        actions: [
          IconButton(
            tooltip: l10n.pwRemoveAllFutureAction,
              ...
              data: (list) => ListView(
                children: [
                  for (final rule in list)
                    Card(
                      child: ListTile(
                        title: Text('${weekdayName(rule.weekday)}  '
  ...
  Future<void> _confirmDeleteRule(
      BuildContext context, WidgetRef ref, PwRecurring rule) async {
    final l10n = context.l10n;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(l10n.commonConfirmDeleteTitle),
        content: Text(l10n.pwRecurringDeleteConfirm),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      await ref.read(pwRepositoryProvider).deleteRecurring(rule.id);
      ...
      builder: (context) {
        ...
        return StatefulBuilder(
          builder: (context, setState) => AlertDialog(
            ...
            title: Text(l10n.pwRemoveAllFutureTitle),
            content: Text(l10n.pwRemoveAllFutureBody),
            actions: [
              TextButton(
              ...
              FilledButton(
                ...
                child: busy
                    ...
                    : Text(l10n.pwRemoveAllFutureConfirm),
              ),
            ],
          ),
        );
  ...
  Future<void> _showRuleDialog(BuildContext context, WidgetRef ref,
    ...
    if (saved == true) {
      // No follow-up write: slots are expanded from the rule, so this edit
      // reaches every one of its occurrences at once.
      await ref
          .read(pwRepositoryProvider)
          .saveRecurring(rule.copyWith(location: locationCtrl.text.trim()));
      ref.invalidate(assignmentHistoryProvider);
    }
